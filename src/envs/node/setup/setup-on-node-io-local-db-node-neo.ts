...
import LocalFileSystem from "../file-system/local-file-system.js";
import getLogger from "./_get-logger.js";

/**
 * `setupOnNodeIoLocalDbDuckdbNodeNeo` のオプションです。
 */
export type SetupOnNodeIoLocalDbDuckdbNodeNeoOptions = Readonly<{
  /**
 ...
 */
export default function setupOnNodeIoLocalDbDuckdbNodeNeo(
  options: SetupOnNodeIoLocalDbDuckdbNodeNeoOptions | undefined = {},
): ISetupFunction {
  return async function setupOnNodeIoLocalDbDuckdbNodeNeoFunction() {
    const {
      json,
      logger,
      rootDir = "omnio",
    ...
    const fs = new LocalFileSystem(rootDir);
    await fs.open();
    ...
    const db = new DuckdbNodeNeo(bucket.path + "/database");
    await db.open();
