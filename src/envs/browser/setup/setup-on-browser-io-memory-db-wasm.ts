...
/**
 * `setupOnBrowserIoMemoryDbDuckdbWasm` のオプションです。
 */
export type SetupOnBrowserIoMemoryDbDuckdbWasmOptions = Readonly<{
  /**
   * DuckDB のロガーです。
   */
 ...
 */
export default function setupOnBrowserIoMemoryDbDuckdbWasm(
  duckdbBundle: DuckdbBundle,
  options: SetupOnBrowserIoMemoryDbDuckdbWasmOptions | undefined = {},
): ISetupFunction {
  return async function setupOnBrowserIoMemoryDbDuckdbWasmFunction() {
    const {
      json,
      logger,
      bucketName = "omnio",
    ...
    const db = new DuckdbWasm(":memory:", duckdbBundle, duckdbLogger);
    await db.open();
